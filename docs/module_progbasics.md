...
#### What is the purpose of a list (array in some programming languages) data structure? Name some methods of it!
    The purpose of a list is to contain values from different types (like strings, intigers, other list, etc.) so you can get your desired result. The list contains values in order, so if you want, you can rearange them as you like (with the .sort()/sorted() function) or you can delete a part from it (with the del() or remove() function) or you can add a new value in to it (with the append() or insert() function).
...
#### What is the purpose and methods of a dictionary/map data structure?
    The dictionary is an immutable data structure, where you can add "keys" in pair with "values". You can give a key a new value, but you can't rename a key. You can't use indexes, you need to use the given key to get the value assigned to the key. The set can only contain hashable values (which is: floats, integers, tuples and strings), but the list can contain non-hashables too (list, set, dictionary)
...
#### What are the main parts of a function?
    The "def" which means define, this part will tell the program that we make a function. The name of the function with a "()" (parentheses) at the end, so the program knows that it's the end of the function's name. Under the name, you can write the statements the function should execute. At the end, you can put a return statement. You can choose not to do that, but in this case, the function will return a "None".
...
#### What arithmetic operators (+,*,-,/) can be used on lists in Python? What do they do?
    You can add (+) lists together, and the result will be the 2 list merged. You can use multiply (*) a list with the times of an integer, and the result will be the list's values.
